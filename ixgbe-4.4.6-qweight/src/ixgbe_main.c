...
#define DRV_VERSION	"4.4.6-qweight" \
			       ...
			       struct ixgbe_ring *tx_ring)
{
	...
	/* I was getting false hangs (and maybe real hangs?)  This solved the
	 * problem.  However, this was in the old slow version of
	 * ixgbe-4.4.6-qweight.  I should go back and see if this can be
	 * renabled. */
	//if (check_for_tx_hang(tx_ring) && ixgbe_check_tx_hang(tx_ring)) {
	if (0) {
...
static inline int ixgbe_maybe_stop_tx(struct ixgbe_ring *tx_ring, u16 size)
{
	 ...
	 *  this driver used a more sophisticated pool->queue mapping in
